...
def index():
    ...
    if request.method == 'POST':
        print(request.form.get('checklist'))
        if request.form.get('checklist') == 'filter':
            interval = timeframe_map[interval_str]
            stocks_select_to_trade.fechdata(interval)
            selectedstocks = readfile().split(",") if readfile() is not None or readfile() != '' else 'no data';

            return render_template('selectedstock.html', symbols=symbols, symbol=symbol, timeframes=timeframes,
                                   timeframe=interval_str,
                                   n_bars=n_bars, rolling_window=rolling_window,
                                   level_diff_threshold=level_diff_threshold,readeddata=selectedstocks.split(selectedstocks)
                                   )
        else:
            ...
            if not df.empty and not level.empty:
                try:
                    # Plot the data
                    mpf.plot(df, type='candle', hlines=level.to_list(), style='charles', savefig='static/chart2.png')
            ...
            return render_template('selectedstock.html', symbols=symbols, symbol=symbol, timeframes=timeframes, timeframe=interval_str,
