...
def sellorerdstetergy(script:any,data:any,interval:any):
    df = data
    ...
    logging.info(f"alredy order place or {filteristoken}")
    ...
    if len(filteristoken) == 0:
        if close_pre > sup_pre and close_cl < sup_cl:
            ...
            if getoptionchain.getparams(symbol, df.close.values[-1], 'pe'):
                ...
                placeoptionsellorder(symbol, stickprice, token, lotsize, ltp, interval, itemclose)
        ...
        elif close_cl > sup_cl and df.low.values[-2] < pivot_fibo_level[s_level] and df.close.values[-1] > pivot_fibo_level[
            s_level] and df.close.values[-1] > df.ema.values[-1] and df.close.values[-1] > df.open.values[-1]:
            if getoptionchain.getparams(symbol, df.close.values[-1],'pe'):
                # GettingLtpData('BANKNIFTY', close_cl, "SELL")
                stickprice = cetoken['symbol']
                lotsize = cetoken['lotsize']
        ...
        elif close_cl < sup_cl and df.high.values[-2] > pivot_fibo_level[r_level] and df.close.values[-1] < \
                pivot_fibo_level[r_level] and df.close.values[-1] < df.ema.values[-1] and df.close.values[-1] < \
                df.open.values[-1]:
            if getoptionchain.getparams(symbol, df.close.values[-1], 'pe'):
                logging.info(f"level 2 : option chain : True ")
                # cetoken,petoken = storetoken.placeorderdetails('NSE', 'OPTIDX', symbol, df.close.values[-1])
            ...
            print('side ways market not match')

    else:  # (if BANKNIFTY order is placed, then run exit script with supertrend)
        print('exit order order here -----', filteristoken)
        ...
        elif df.close.values[-1] < df.ema.values[-1]:
                        print('exit --- 3')
        ...
        elif ('pe' in buytradedBANKNIFTY) and get_strick_price_historicaldata.checkexitconditio(pe_symbol['symbol'],
                                                                                                pe_symbol['token']):
        ...
        elif ('ce' in buytradedBANKNIFTY) and get_strick_price_historicaldata.checkexitconditio(ce_symbol['symbol'],
                                                                                                ce_symbol['token']):
                ...
                for interval, key in timeframe_map.items():
                    try:
                        ...
                        if result1:
                            ...
                            closelow = df_5min.low.values[-2]
                            change = closehigh - closelow
                            if (change >= 20 and script == 'NIFTY') or (change >= 40 and script == 'BANKNIFTY') or (change >= 30 and script == 'CNXFINANCE'):
                                # merged_array = level + pivot_fibo_level  # Merging the two arrays
